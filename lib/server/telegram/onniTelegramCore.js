...
const START_TEXT = [
  ...
  "• enviar correos (SMTP, uno por uno)",
  "",
...
function buildHelpText(env) {
  return [
    ...
    "",
    "Correos (SMTP Gmail):",
    "• enviar correos → asunto, mensaje, lista, adjunto, confirmar",
    isSmtpConfigured(env)
      ? `• SMTP: configurado (${env.SMTP_USER || "ok"})`
      : "• SMTP: falta SMTP_USER y SMTP_PASS en .env.local",
    "",
