...
import { sendGmailBulk } from "../gmail/gmailSend.js";
...
function gmailSetupMessage(env) {
  const startUrl = getGmailOAuthStartUrl(env);
  return [
    "Gmail aún no está autorizado en este PC.",
    "",
    "1. Abre en el navegador:",
    startUrl,
    "2. Inicia sesión con tu Gmail y acepta permisos.",
    "3. Copia GMAIL_REFRESH_TOKEN en .env.local junto con:",
    "   GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET, GMAIL_USER=tu@gmail.com",
    "4. Reinicia npm run telegram:bot",
    "",
    "Luego escribe enviar correos otra vez.",
  ].join("\n");
}
...
async function executeSend(session, chatId, token, env) {
  ...
  const progressLines = [];
  const { sent, failed, total, results } = await sendGmailBulk(
    {
